...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in shortVariant) {
      if (
        !shortVariant[item.title].color.includes[item.color] &&
        item.availableQty > 0
      ) {
        shortVariant[item.title].color.push(item.color);
      }
      if (
        !shortVariant[item.title].size.includes[item.size] &&
        item.availableQty > 0
      ) {
        shortVariant[item.title].size.push(item.size);
      }
    } else {
      shortVariant[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        shortVariant[item.title].color = [item.color];
        shortVariant[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(shortVariant)) },
  };
}
